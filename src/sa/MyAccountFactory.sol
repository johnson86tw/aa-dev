...
contract MyAccountFactory {
     ...
     */
    function createAccount(uint256 salt) public returns (MyAccount ret) {
        ...
        ret = MyAccount(
            payable(
                new ERC1967Proxy{salt: bytes32(salt)}(
                    address(accountImplementation), abi.encodeCall(MyAccount.initialize, ())
                )
            )
        );
    }
        ...
        return Create2.computeAddress(
            ...
            keccak256(
                abi.encodePacked(
                    ...
                    abi.encode(address(accountImplementation), abi.encodeCall(MyAccount.initialize, ()))
                )
            )
        );
    }
